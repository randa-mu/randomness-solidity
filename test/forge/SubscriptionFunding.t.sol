...
contract SubscriptionFundingTest is RandomnessTest {
    ...
    function test_FulfillSignatureRequest_WithGasPriceHigherThanSubscriptionBalance_ShouldRevert() public {
        // create subscription and fund it
        assert(mockRandomnessReceiver.subscriptionId() == 0);
        ...
        // This should cause a revert with "InsufficientBalance()" because it
        // will cause this logic in the contract to fail where
        // subscription balance cannot cover tx.gasPrice * gas units / cost for the entire callback + gas overheads
        // uint96 payment = _calculatePaymentAmountNative(startGas, tx.gasprice);
        // _chargePayment(payment, request.subId);
        vm.txGasPrice(requestPrice);
        uint256 gasBefore = gasleft();
