...
const G1_SVDW = mapToCurveSVDW(_bn254.G1.CURVE, {
  ...
  // c1 = g(Z)
  c1: _bn254.G1.CURVE.Fp.create(BigInt(4)),
  // c2 = -Z / 2
  c2: _bn254.G1.CURVE.Fp.create(
    BigInt("10944121435919637611123202872628637544348155578648911831344518947322613104291"),
  ),
  // c3 = sqrt(-g(Z) * (3 * Z^2 + 4 * A))     # sgn0(c3) MUST equal 0
  c3: _bn254.G1.CURVE.Fp.create(BigInt("8815841940592487685674414971303048083897117035520822607866")),
  // c4 = -4 * g(Z) / (3 * Z^2 + 4 * A)
  c4: _bn254.G1.CURVE.Fp.create(BigInt("7296080957279758407415468581752425029565437052432607887563012631548408736189")),
});
...
const G2_SVDW = mapToCurveSVDW(_bn254.G2.CURVE, {
  ...
  c1: _bn254.G2.CURVE.Fp.create({
    c0: BigInt("19485874751759354771024239261021720505790618469301721065564631296452457478374"),
    c1: BigInt("266929791119991161246907387137283842545076965332900288569378510910307636690"),
  }),
  // c2 = -Z / 2
  c2: _bn254.G2.CURVE.Fp.create({
    c0: BigInt("10944121435919637611123202872628637544348155578648911831344518947322613104291"),
    c1: BigInt(0),
  }),
  // c3 = sqrt(-g(Z) * (3 * Z^2 + 4 * A))     # sgn0(c3) MUST equal 0
  c3: _bn254.G2.CURVE.Fp.create({
    c0: BigInt("18992192239972082890849143911285057164064277369389217330423471574879236301292"),
    c1: BigInt("21819008332247140148575583693947636719449476128975323941588917397607662637108"),
  }),
  // c4 = -4 * g(Z) / (3 * Z^2 + 4 * A)
  c4: _bn254.G2.CURVE.Fp.create({
    c0: BigInt("10499238450719652342378357227399831140106360636427411350395554762472100376473"),
    c1: BigInt("6940174569119770192419592065569379906172001098655407502803841283667998553941"),
  }),
});
...
const { G2psi, G2psi2, psi } = psiFrobenius(
  _bn254.fields.Fp,
  _bn254.fields.Fp2,
  _bn254.fields.Fp2.create({ c0: BigInt(9), c1: BigInt(1) }),
); // 9 + i is the nonresidue
...
export const bn254: CurveFn = bls({
  ...
  fields: {
    Fp: _bn254.fields.Fp,
    Fp2: _bn254.fields.Fp2,
    Fp6: _bn254.fields.Fp6,
    Fp12: _bn254.fields.Fp12,
    Fr: _bn254.fields.Fr,
  },
  G1: {
    ..._bn254.G1.CURVE,
  ...
  G2: {
    ...
    mapToCurve: (scalars: bigint[]) => {
      const u = _bn254.G2.CURVE.Fp.create({ c0: scalars[0], c1: scalars[1] });
      return G2_SVDW(u);
    },
