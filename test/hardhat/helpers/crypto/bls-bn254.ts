...
import { dataSlice, hexlify, getBytes, keccak256, randomBytes, solidityPacked, BytesLike, isHexString } from "ethers";
 ...
 */
class BlsBn254 {
  static readonly FIELD_ORDER = 0x30644e72e131a029b85045b68181585d97816a916871ca8d3c208c16d87cfd47n;
...
export function kyberG1ToEvm(g1: Uint8Array): [bigint, bigint] {
  const p = [g1.slice(0, 32), g1.slice(32, 64)].map((sigBuf) => BigInt(hexlify(sigBuf))) as [bigint, bigint];
  return p;
}
...
export function serialiseG2Point(p: G2): [bigint, bigint, bigint, bigint] {
  ...
  return [BigInt(dataSlice(x, 32)), BigInt(dataSlice(x, 0, 32)), BigInt(dataSlice(y, 32)), BigInt(dataSlice(y, 0, 32))];
}
