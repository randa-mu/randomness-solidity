...
contract ChainlinkVRFCoordinatorV2_5Stub is VRF, SubscriptionAPI, IVRFCoordinatorV2Plus {
        ...
        returns (uint256 requestId)
    {
        ...
        s_requestCommitments[requestId] = keccak256(
            abi.encode(
                requestId, _getBlockNumber(), subId, req.callbackGasLimit, req.numWords, msg.sender, extraArgsBytes
            )
        );
    ...
    function _computeRequestId(bytes32 keyHash, address sender, uint256 subId, uint64 nonce)
        internal
        pure
    ...
    function _getRandomnessFromProof(Proof calldata proof, VRFTypes.RequestCommitmentV2Plus calldata rc)
        internal
        view
    ...
    */
    function fulfillRandomWords(Proof calldata proof, VRFTypes.RequestCommitmentV2Plus calldata rc, bool onlyPremium)
        external
        nonReentrant
    ...
    function _calculatePaymentAmount(uint256 startGas, uint256 weiPerUnitGas, bool nativePayment, bool onlyPremium)
        internal
        returns (uint96, bool)
    {
        if (nativePayment) {
    ...
    function _calculatePaymentAmountNative(uint256 startGas, uint256 weiPerUnitGas, bool onlyPremium)
        internal
        returns (uint96)
    {
    ...
    function _calculatePaymentAmountLink(uint256 startGas, uint256 weiPerUnitGas, bool onlyPremium)
        internal
        returns (uint96, bool)
    {
